...
- Additional v2 part subtypes (`subtask`, `agent`, `retry`, `compaction`, `snapshot`, `patch`) MUST be preserved in reducer state rather than dropped on ingest; only the user-visible-safe subset MAY be surfaced through snapshot text/status semantics.
- Internal orchestration markers and control payloads (`OMO_INTERNAL_INITIATOR`, `<system-reminder>`, background-task completion directives, TODO-continuation directives, and compaction handoff summaries) MUST NOT be materialized into downstream user-visible transcript blocks even when they appear in upstream assistant text or raw final-output payloads.
...
- Gateway continuity-warning policy SHOULD suppress repeated identical reused-session warnings for the same persisted session ID after the user has already been warned once; a new session ID, forked-session handoff, or explicit continuity reset MUST re-arm the warning.
...
- Tunnel runtime state MUST verify that a tracked ngrok listener still exists before reusing or reporting it; stale listener records MUST be evicted instead of advertising dead public URLs.
- Tunnel open handling SHOULD retry exactly once for transient ngrok control-session failures that occur before listener startup completes (for example muxado stream open failures), after clearing stale listener/session state.
